...
class TestProject:
    ...
    def sync_case_tree_node_info_for_testplans(self):
        ...
        try:
            ...
            query = 'SELECT node_id FROM `website_ui_case_tree_test_plan` WHERE plan_id IN ' + plan_id_list + ' GROUP BY node_id'
            data = ''
            ...
            if result[0] and result[1]:
                ...
                for record in records:
                    ...
                    query = "SELECT t1.text, t1.parent_id, COUNT(t2.id) FROM `website_ui_case_tree` AS t1 " \
                            "LEFT JOIN `website_ui_case_tree` AS t2 ON t2.parent_id = t1.id WHERE t1.id = %s"
                    ...
                    if result[0] and result[1]:
                        ...
                        query = "UPDATE website_ui_case_tree_test_plan SET node_path='%s', sub_node_num=%s WHERE plan_id IN %s AND node_id = %s"
